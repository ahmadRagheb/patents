...
// For license Patent, please see license.txt
frappe.ui.form.on("Patent", {
	...
	onload: function(frm) {
		var so = frappe.meta.get_docfield("Patent", "sales_order");
		...
		frm.set_query("user", "users", function() {
			return {
				query:"patents.Patents.doctype.patent.patent.get_users_for_project"
			}
		});
	...
	refresh: function(frm) {
		if(frm.doc.__islocal) {
			...
			frm.add_web_link("/Patents?Patent=" + encodeURIComponent(frm.doc.name));

			if(frappe.model.can_read("Task")) {
				frm.add_custom_button(__("Gantt Chart"), function() {
					frappe.route_options = {"Patent": frm.doc.name};
