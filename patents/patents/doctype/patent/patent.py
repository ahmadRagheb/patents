# -*- coding: utf-8 -*-
# Copyright (c) 2018, ahmedragheb and contributors
...
class Patent(Document):
	...
	def get_tasks(self):
		if self.name is None:
			...
			filters = {"patent_project": self.name}
	...
	def sync_tasks(self):
		...
		# delete
		for t in frappe.get_all("Task", ["name"], {"patent_project": self.name, "name": ("not in", task_names)}):
	...
	def after_insert(self):
		if self.sales_order:
			frappe.db.set_value("Sales Order", self.sales_order, "patent_project", self.name)


	def update_percent_complete(self):
		total = frappe.db.sql("""select count(name) from `tabTask` where patent_project=%s""", self.name)[0][0]
		...
		if (self.percent_complete_method == "Task Completion" and total > 0) or (not self.percent_complete_method and total > 0):
			completed = frappe.db.sql("""select count(name) from `tabTask` where
				patent_project=%s and status in ('Closed', 'Cancelled')""", self.name)[0][0]
			self.percent_complete = flt(flt(completed) / total * 100, 2)

		if (self.percent_complete_method == "Task Progress" and total > 0):
			progress = frappe.db.sql("""select sum(progress) from `tabTask` where
				patent_project=%s""", self.name)[0][0]
		...
		if (self.percent_complete_method == "Task Weight" and total > 0):
			...
			if weight_sum == 1:
				weighted_progress = frappe.db.sql("""select progress,task_weight from `tabTask` where
					patent_project=%s""", self.name,as_dict=1)
				pct_complete=0
	...
	def update_costing(self):
		...
		from_expense_claim = frappe.db.sql("""select
			...
			from `tabExpense Claim` where patent_project = %s and approval_status='Approved'
			and docstatus = 1""",
	...
	def update_purchase_costing(self):
		total_purchase_cost = frappe.db.sql("""select sum(base_net_amount)
			from `tabPurchase Invoice Item` where patent_project = %s and docstatus=1""", self.name)

		self.total_purchase_cost = total_purchase_cost and total_purchase_cost[0][0] or 0

	def update_sales_amount(self):
		total_sales_amount = frappe.db.sql("""select sum(base_grand_total)
			from `tabSales Order` where patent_project = %s and docstatus=1""", self.name)

		self.total_sales_amount = total_sales_amount and total_sales_amount[0][0] or 0
	#NM
	def update_billed_amount(self):
		total_billed_amount = frappe.db.sql("""select sum(base_grand_total)
			from `tabSales Invoice` where patent_project = %s and docstatus=1""", self.name)

		self.total_billed_amount = total_billed_amount and total_billed_amount[0][0] or 0

	def after_rename(self, old_name, new_name, merge=False):
		if old_name == self.copied_from:
			frappe.db.set_value('Patent', new_name, 'copied_from', new_name)

	def send_welcome_email(self):
		url = get_url("/Patent/?name={0}".format(self.name))
		messages = (
		...
		if self.name != self.copied_from and self.get('__unsaved'):
			...
			for task in self.tasks:
				_task = frappe.db.get_value(
					'Task',
					{"subject": task.title, "patent_project": self.copied_from},
			...
			for key, value in dependency_map.iteritems():
				task_name = frappe.db.get_value('Task', {"subject": key, "patent_project": self.name })
				task_doc = frappe.get_doc('Task', task_name)

				for dt in value:
					dt_name = frappe.db.get_value('Task', {"subject": dt, "patent_project": self.name })
...
def get_project_list(doctype, txt, filters, limit_start, limit_page_length=20, order_by="modified"):
	return frappe.db.sql('''select distinct project.*
		from tabPatent project, `tabProject User` project_user
		where
...
def get_cost_center_name(project):
	return frappe.db.get_value("Patent", project, "cost_center")
